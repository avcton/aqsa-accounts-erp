...
const generatePDF = vouchers => {
  ...
  const tableColumn = ["Voucher No", "Period Code", "Vocuher Date", "Account Code", "Debit Amount", "Credit Amount", "Particulars", "CreatedBy", "PostedBy"];
  ...
  vouchers.forEach(voucher => {
    const voucherData = [
      ...
      voucher.PeriodCode,
      getFormattedDate(voucher.VoucherDate),
      voucher.AccountCode,
      voucher.DebitAmount,
...
const generateExcelSheet = async vouchers => {
  ...
  exportToCSV(data, `Payables_ExcelSheet`);
}
...
export default function ReceivablesReport() {
  ...
  const getVouchers = async () => {
    ...
    return await fetch(`${baseURL}/api/receivables?date=${formattedDate}`, {
      method: 'GET',
      headers: {
    ...
    <div className=" bg-slate-50 z-20">
      <motion.div initial={{ opacity: 0, y: 50 }}
        ...
        <div className=' flex flex-col space-y-5 items-center justify-center mb-32'>
          ...
          <h3 className=" text-black text-3xl font-bold mb-6 mt-14 ">Receivables Report</h3>
              ...
              <TableBody>
                {
                  vouchers.map((voucher) => (
                    <StyledTableRow key={voucher.VoucherNumber}>
                      ...
                      <StyledTableCell align='center'>{getFormattedDate(voucher.VoucherDate, 'true')}</StyledTableCell>
