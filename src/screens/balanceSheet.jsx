...
function createData(Assets, Liabilities, OwnersEquity) {
  return { Assets, Liabilities, OwnersEquity };
}

const rows = [
  createData('Current', 'Current', 'Contributed Capital')
  ...
  return (
    <div className='flex flex-col h-screen w-screen align items-center justify-center bg-slate-50 overflow-auto'>
      <div className=' flex flex-col items-center justify-center '>
        <h3 className=" text-black text-3xl font-bold mb-6 ">Balance Sheet</h3>
          ...
          <Table sx={{ minWidth: 350 }} aria-label="customized table">
            <TableHead>
              <TableRow>
                ...
                <StyledTableCell align="center">Owners Equity</StyledTableCell>

              </TableRow>
            </TableHead>
            <TableBody>
              {rows.map((row) => (
                <StyledTableRow key={row.Assets}>
                  <StyledTableCell component="th" scope="row">
                    {row.Assets}
                  </StyledTableCell>
                  <StyledTableCell align="center">{row.Liabilities}</StyledTableCell>
                  <StyledTableCell align="center">{row.OwnersEquity}</StyledTableCell>
                </StyledTableRow>
              ))}
            </TableBody>
          </Table>
